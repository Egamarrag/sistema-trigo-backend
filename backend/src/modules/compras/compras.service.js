...
async function listarCompras() {
  return prisma.compra.findMany({
    include: { proveedor: true, origen: true, lotesCompra: true, cuentaPorPagar: true },
...
async function registrarCompra(datos) {
  const { proveedorId, origenId, fechaCompra, pesoBrutoKg, precioKg, humedadPct, condicionPago, usuarioId } = datos;
  ...
  const resultado = await prisma.$transaction(async (tx) => {
    const compra = await tx.compra.create({
      data: {
        ...
        origenId: origenId ? Number(origenId) : null,
